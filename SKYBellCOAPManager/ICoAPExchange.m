...
- (ICoAPMessage *)decodeCoAPMessageFromData:(NSData *)data {
    ...
    NSUInteger tokenLength = strtol([[hexString substringWithRange:NSMakeRange(1, 1)] UTF8String], NULL, 16); // in Bytes
    ...
    while (isOptionLoopRunning) {
        if (optionIndex + 2 < [hexString length]) {
            NSUInteger optionDelta = strtol([[hexString substringWithRange:NSMakeRange(optionIndex, 1)] UTF8String], NULL, 16);
            NSUInteger optionLength = strtol([[hexString substringWithRange:NSMakeRange(optionIndex + 1, 1)] UTF8String], NULL, 16);
...
#if DEBUG
//    NSLog(@"%@",[NSString stringFromDataWithHex:result]);
#endif
    return result;
}
